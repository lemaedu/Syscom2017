...
class Crud {
    ...
    // Atributo para guardar una   
    private $pdo = null;
    // Atributo que guardara el nombre de la tabla    
    private $tabela = null;
    // Atributo estática que contiene una instanacia propia de una clase   
    private static $crud = null;

    /*
     * Método privado construtor da classe    
     * @param $conexion = Conexion PDO configurada   
     * @param $tabela = Nome da tabela    
     */

    private function __construct($conexion, $tabela = NULL) {
     ...
     * @param $conexion = Conexão PDO configurada   
     * @param $tabela = Nome da tabela   
     * @return Atributo contendo instância da classe Crud   
     */

    public static function getInstance($conexion, $tabela = NULL) {

        // Verifica se existe uma instância da classe   
        if (!isset(self::$crud)):
            try {
                self::$crud = new Crud($conexion, $tabela);
            } catch (Exception $e) {
